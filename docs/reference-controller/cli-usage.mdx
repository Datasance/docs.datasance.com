---
title: Legacy Controller CLI Usage - Reference
description: Legacy CLI command reference for iofog-controller. Frozen for v1.6.x; use potctl on v3.8.
keywords: [controller cli, iofog-controller, legacy cli, datasance pot, control plane management, command line interface]
---

# Legacy Controller CLI Usage

:::warning[Legacy only - not v3.8]

This page documents the removed **`iofog-controller`** CLI from v1.6.x. Controller **v3.8.0** uses [{CLI_NAME}](/potctl/introduction) and the [REST API](/reference-controller/rest-api). Content is kept for deep links from [potctl legacy](/v3.7.3/potctl/legacy).

:::

import { CLI_NAME } from '@site/src/config/distribution';

Use [{CLI_NAME}](/potctl/introduction) for current CLI usage.
