...
**Streaming logs:** Agent and microservice logs can also be streamed via **WebSocket** (exec-style session), so you can tail logs in real time without polling. This is supported in **[EdgeOps Console](/edgeops-console/features)** (browser-based terminal and log views) and in **potctl** (e.g. when using log or exec-style commands). The same WebSocket channel is used for interactive exec sessions and log streaming.
