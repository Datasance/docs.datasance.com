...
| kind               | String representing what type of resource we want to deploy. The available values are `ControlPlane`, `KubernetesControlPlane`, `LocalControlPlane`, `Controller`, `Agent`, `LocalAgent`, `AgentConfig`, `Registry`, `CatalogItem`, `Application`, `Microservice`, `Secret`, `Certificate`, `CertificateAuthority`, `ConfigMap`, `VolumeMount`, `Service`, `OfflineImage`  and `Volume`. |
...
| metadata.tags | User defined tags can be defined for `Agent`, `LocalAgent`, `AgentConfig` and `Service` kinds.                                                                                                                                                                                                              |
| spec               | Object containing the deployment specifications, different for each resource    
| data               | Object containing the data fields for `Secret` and `ConfigMap` kinds                                                                                                                                                                                                 |
