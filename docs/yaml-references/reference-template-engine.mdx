---
title: Template Engine for YAML - potctl Reference
description: Guide to using variables, filters, and templates in potctl YAML files. Dynamic configuration for Datasance PoT resources.
keywords: [template engine, potctl, dynamic yaml, liquidjs, datasance pot, edge configuration, yaml templates]
---
  ...
  microservices:
    - name: rulesengine
      agent:
        ...
        config: {}
      images:
        amd64: nodered/node-red:latest
        arm64: nodered/node-red:latest
        registry: remote
      config: {}
      container:
