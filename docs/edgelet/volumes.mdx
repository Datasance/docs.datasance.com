---
title: Edgelet Volumes
description: How Edgelet stores microservice volume data. VOLUME vs BIND vs VOLUME_MOUNT, delete behavior, and prune on Edgelet nodes.
keywords: [edgelet, volumes, volume, prune, disk, VOLUME_MOUNT, persistent data]
slug: /edgelet/volumes
---
...
Image layers and container filesystems live under `/var/lib/edgelet-containerd/` (embedded engine), **not** under `volumes/data/`. Backing up `edgelet.db` alone does **not** back up `VOLUME` data. See [Persistence](/edgelet/persistence).
...
| **`VOLUME_MOUNT`** | Controller volume name | Materializes under `volumes/microservices/{ms-uuid}/` from shared `secrets/` or `configMaps/` trees |
| **`BIND`** | Absolute host path, e.g. `/opt/data` | Bind-mounts that path; Edgelet never deletes it |

Local deploy manifests (`edgelet deploy -f`) support **`BIND`** and **`VOLUME`** only. Controller workloads may also use **`VOLUME_MOUNT`** for secrets and configmaps synced from the Controller.

## What happens when a microservice is deleted

Deletion paths:

- **Controller:** microservice marked `delete: true` in Controller snapshot → process manager reconciles removal
...
When the Controller sets `deleteWithCleanup: true` on a microservice:
...
`PruneVolumes` removes orphaned directories under `volumes/data/{uuid}/` and `volumes/microservices/{uuid}/` for any microservice UUID that has **no running container**.

Triggered by:

- Scheduled prune (`pruningFrequency` in config) — runs containers → volumes → images
- Manual prune:
...
| Wipe node completely | Stop services, remove `diskDirectory` (see [Persistence](/edgelet/persistence)) |
...
2. Confirm no container still references the UUID: `edgelet ms ls`
3. Prune orphans: `edgelet system prune volumes`

### Redeployed microservice has empty volume

`VOLUME` data is keyed by **microservice UUID**. A new deployment with a new UUID gets a **new** directory. To preserve data across UUID changes, use **`BIND`** to a fixed host path you control.

## See also

- [Deploy manifests](/edgelet/deploy-manifests) — `spec.container.volumes` YAML fields
- [Persistence](/edgelet/persistence) — SQLite backup scope
- [Container engines](/edgelet/container-engines) — `diskDirectory` vs containerd paths
- [Configuration reference](/edgelet/configuration-reference) — `pruningFrequency`, `availableDiskThreshold`
