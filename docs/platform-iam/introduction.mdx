...
By default preconfigured Datasance PoT realm comes with  two client `pot-controller` and `ecn-viwer`. 
...
- `Viewer`'s can only viewe the resources on cluster.
...
:::warning[Don't have Keycloak Intance?]
<p>We recommened going through the <a href="../platform-iam/keycloak-deployment">Keycloak Deployment Guide</a> before continuing on here.</p>
::: 
...
Create user inside imported realm. Create user's password and assing role.
...
## Configure ECN-Viwer URL

:::warning[Configure ECN-Viewer Redirect URL]
Once you deploy your Datasance PoT `ControlPlane`, you need to configure ECN-Viwer redirect URL on Keycloak client setting.
