---
title: HAL Microservice - Catalog Reference
description: Hardware Abstraction Layer (HAL) microservice reference. Access hardware capabilities like GPIO, USB, and CPU info via REST/WS APIs.
keywords: [hal, hardware abstraction layer, gpio, usb, lscpu, edge hardware access, microservice, peripheral access]
---
...
> You can use the `hostNetworkMode` and  `isPrivileged` fields of the [microservice yaml definition](../reference-potctl/reference-application) to grant root access to the host to your container
