---
title: NATS JWT Authentication
description: How Datasance PoT uses NATS JWT authentication. Operator, account, and user JWTs; how the Controller provisions credentials for microservices.
keywords: [nats, jwt, authentication, messaging, datasance pot, natsAccess, natsRule, credentials]
---

# NATS JWT Authentication

When NATS is enabled on a control plane, Datasance PoT uses **NATS JWT authentication** so that microservices and system components connect to NATS with cryptographically signed identities. The Controller (and optionally potctl) generates or distributes these credentials so you can deploy microservices with NATS access without manually creating or rotating JWTs.

## JWT hierarchy

NATS uses a three-level JWT model:

1. **Operator JWT** – Identifies the NATS operator (the PoT deployment). The Controller manages the operator identity.
2. **Account JWT** – Identifies a NATS account. Each account has limits and import/export rules. In PoT, an account is represented by a [NATS Account Rule](nats-account-rule); the Controller creates the account JWT from that rule.
3. **User JWT** – Identifies a user within an account (e.g. a microservice). In PoT, a user is represented by a [NATS User Rule](nats-user-rule); the Controller creates the user JWT (or a creds file containing it) from that rule.
...
```bash
potctl nats operator describe --jwt
aud: NATS
iat: 1.772706195e+09
...
1. You define a [NATS Account Rule](nats-account-rule) and one or more [NATS User Rules](nats-user-rule) (via the Controller REST API or YAML).
...
potctl deploy -f /tmp/account-export.yaml
```
...
potctl deploy -f /tmp/account-import.yaml
```
...
spec:
  ...
  microservices:
    - name: box-1
      ...
      images:
        registry: 1
        amd64: natsio/nats-box:latest
        arm64: natsio/nats-box:latest
      container:
...
```bash
potctl describe nats-account nats-test
id: 5
name: nats-test
...
```bash
potctl describe nats-account nats-test3 --jwt
aud: NATS
iat: 1.772706624e+09
iss: OCRQSRFBZEBHQQMQ6N37KCBSZCQI2WEG7DQXHRXUUW3FSBBN5TP2ZDLV
...
```bash
potctl describe nats-account nats-test2 --jwt
aud: NATS
iat: 1.772706721e+09
iss: OCRQSRFBZEBHQQMQ6N37KCBSZCQI2WEG7DQXHRXUUW3FSBBN5TP2ZDLV
...
```bash
potctl describe nats-account nats-test --jwt
aud: NATS
iat: 1.772706721e+09
iss: OCRQSRFBZEBHQQMQ6N37KCBSZCQI2WEG7DQXHRXUUW3FSBBN5TP2ZDLV
...
```bash
potctl describe microservice nats-test/box-1
apiVersion: datasance.com/v3
kind: Microservice
metadata:
...
```bash
potctl nats user creds nats-test box-1
-----BEGIN NATS USER JWT-----
...
```bash
potctl describe nats-user nats-test box-1 --jwt
aud: NATS
iat: 1.772706738e+09
...
PoT handles infrastructure and security for you so you can focus on the innovation and intelligence you bring to the edge.

## TLS for NATS connections

NATS client and leaf connections are secured with TLS. PoT provides secure NATS clusters by default, so you do not need to configure TLS between NATS instances yourself.

## Related documentation

- [NATS Account Rule](nats-account-rule) – Account-level policy and limits. YAML spec: [NatsAccountRule YAML](../yaml-references/reference-nats-account-rule).
- [NATS User Rule](nats-user-rule) – User-level policy for microservices. YAML spec: [NatsUserRule YAML](../yaml-references/reference-nats-user-rule).
- [Application YAML – NATS access](../yaml-references/reference-application#nats-access-natsconfig) – Enabling NATS on applications and microservices.
- [Control Plane YAML](../yaml-references/reference-control-plane) – Enabling NATS on the control plane.
