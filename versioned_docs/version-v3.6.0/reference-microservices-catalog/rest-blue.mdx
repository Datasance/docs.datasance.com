---
title: Bluetooth REST API Microservice - Catalog Reference
description: Bluetooth Low Energy (BLE) microservice reference. Scan, connect, and interact with BLE devices via REST API.
keywords: [bluetooth rest api, ble, edge bluetooth, restblue, microservice, iot connectivity, bluetooth gateway]
---
...
> You can use the `hostNetworkMode` and  `isPrivileged` of the [microservice yaml definition](../reference-potctl/reference-application) to grant root access to the host to your container
