...
description: Reference for Control Plane YAML specifications. Deploy Datasance PoT Control Plane on Kubernetes or remote hosts.
keywords: [control plane yaml, kubernetes control plane, remote control plane, potctl, datasance pot, edge orchestration, ecn setup]
---

# Control Plane YAML Specification

`potctl` allows users to deploy Edge Compute Networks (ECNs). The resources that make up an ECN—Controller, Router, and optionally NATS—are defined in YAML. This page covers three control plane kinds: **KubernetesControlPlane** (for Kubernetes clusters), **ControlPlane** (for remote hosts), and **LocalControlPlane** (for local Docker). When NATS is enabled, the control plane runs NATS server(s); applications and microservices use [NATS Account Rules](../security/nats-account-rule) and [NATS User Rules](../security/nats-user-rule) for access. For TLS and certificates, see [Certificates for Router and NATS](../security/certificates-manager).

:::warning[potctl and ECN version compatibility]
  <p>You can connect to an older ECN with potctl +v1.6.0 for view-only purposes. If you would like full control over the ECN with potctl +v1.6.0, you should delete the ECN with your older potctl and then redeploy with potctl +v1.6.0 .</p>
:::

## Kubernetes Control Plane

The **KubernetesControlPlane** kind deploys the Controller, Router, and (optionally) NATS on a Kubernetes cluster. The operator manages the lifecycle; you configure replicas, images, services, ingresses, and optional vault and database.
...
spec:
  ...
  images:
   # pullSecret:  # set the K8S pull secret name
    ...
    nats: ghcr.io/datasance/nats:2.12.4   # used when NATs is enabled
...
| controller      | Controller-specific settings: `ecnViewerUrl`, `https`, `secretName`, `logLevel`. See [Controller Configuration](../reference-controller/configuration) for full config options.                                                                                                                     |
...
:::warning[Router and NATS ingress]
  Ingress settings for Router and NATS configure the Control Plane's advertised addresses and ports; they do not necessarily create Kubernetes Ingress resources depending on your setup.
:::

:::warning[Router and NATS TLS]
  By default, Router and NATS instances are deployed with TLS. If you do not provide your own CA, the operator creates self-signed certificates. To use your own CA, create Kubernetes secrets `pot-site-ca` and `default-router-local-ca` in the Control Plane namespace. See [Certificates for Router and NATS](../security/certificates-manager).
:::

## Remote Control Plane

The **ControlPlane** kind deploys the Controller (and optionally Router, NATS, and a System Agent) on one or more remote hosts. potctl uses SSH to install and configure each Controller. You can enable NATS with `nats.enabled: true` and optionally set `airgap: true` for air-gapped installs.
...
spec:
  ...
  #   container:
  #     image: emirhandurmus/controller:3.7.0
  # systemMicroservices:
  #   router:
...
| controller.container.image | Docker image for the Controller (e.g. ghcr.io/datasance/controller:3.7.0). |
| auth             | Authentication configuration for Keycloak: url, realm, realmKey, ssl, controllerClient, controllerSecret, viewerClient. |       
