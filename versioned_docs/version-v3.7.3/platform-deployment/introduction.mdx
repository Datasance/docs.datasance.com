...
keywords: [platform deployment, datasance pot, kubernetes edge, remote deployment, potctl, edge compute network, ecn setup]
---

:::warning[Prepare your Keycloak Realms for Datasance PoT]
  <p>We recommened going through the <a href="../platform-deployment/prepare-realm">Keycloak Installation Guide</a> before continuing on here. In this guide we also assume we are up and running with potctl, if that is not the case download and be at least marginally familiar with <a href="../potctl/download">potctl</a>.</p>
:::
...
For deployments in **air-gapped** or restricted networks where hosts cannot access the internet or container registries, see [Airgap Deployment](airgap-deployment).
