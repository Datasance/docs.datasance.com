...
4		Debug		The built-in debugger for Datasance PoT IoFog Agent.	remote		ghcr.io/datasance/node-debugger:latest	ghcr.io/datasance/node-debugger:latest	
1		Router		The built-in router for Datasance PoT.			remote		ghcr.io/datasance/router:3.7.0		ghcr.io/datasance/router:3.7.0		
...
" > /tmp/debugger.yaml
potctl deploy microservice -f /tmp/debugger.yaml
```
...
We can also use potctl to create our own Catalog Items. The YAML spec reference can be found [here](../yaml-references/reference-catalog).
