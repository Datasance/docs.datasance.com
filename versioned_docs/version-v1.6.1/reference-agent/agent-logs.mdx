---
title: Agent Logs - Reference
description: Troubleshooting with Agent logs. Understand log formats, levels, and storage locations for Datasance PoT Agents.
keywords: [agent logs, troubleshooting, log format, datasance pot, edge agent, edge debugging]
---
...
**Streaming logs:** Agent logs can be streamed via **WebSocket** (exec-style session), similar to microservice logs. ECN-Viewer and potctl support this so you can tail Agent logs in real time. See [Microservice Logs](../applications/microservice-logs) and the ECN-Viewer documentation for the unified log/exec experience.

```bash

potctl logs agent --help
