...
:::warning[Legacy tutorial: ioMessage and Local API deprecated]
  This tutorial uses the **ioFog SDK** with the legacy **ioMessage** format and **Local API** for microservice communication. These are **deprecated**. For new development, use **NATs** for messaging (enable NATs on the control plane and use [NATs Account Rule](../security/nats-account-rule) and [NATs User Rule](../security/nats-user-rule)). This page is kept for legacy reference.
:::
...
Now we need to install the ioFog SDK for Node.js, which is published to NPM as [@datasance/iofog-nodejs-sdk](https://www.npmjs.com/package/@datasance/iofog-nodejs-sdk):

```sh
npm install --save @datasance/iofog-nodejs-sdk
```
...
```json
{
  ...
  "license": "ISC",
  "dependencies": {
    "@datasance/iofog-nodejs-sdk": "3.0.1"
  }
}
```
...
```js
const iofog = require('@datasance/iofog-nodejs-sdk');
```

#### iofog.init()


:::danger[Use this as our entry point]
