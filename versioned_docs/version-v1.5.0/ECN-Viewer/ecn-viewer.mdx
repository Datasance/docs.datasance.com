...
Track and audit all Controller API endpoint calls through the Events interface. Filter events by type, endpoint, status, method, resource type, actor, and time range. Export event logs for compliance reporting or delete old events to manage storage.

![Events log with comprehensive filtering and audit trail](/images/ecn-viewer/events.png)
