---
title: Agent YAML Specification - potctl Reference
description: YAML specification and configuration reference for Datasance PoT Agents. Manage edge node behavior, security, and settings.
keywords: [agent yaml, edge agent, datasance pot, potctl, agent configuration, edge node, edgeguard]
---
...
spec:
  ...
  ssh:
    ...
    port: 22
  package:
    # version: 3.6.0# provide Agent package version if you would like to deploy Agent as a deb or rpm package
    container: 
      image: ghcri.io/datasance/agent:3.6.0 # provide image in the case you would like to deploy Agent as a container
  config:
